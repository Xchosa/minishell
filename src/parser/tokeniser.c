...
/*   Updated: 2025/04/15 14:07:11 by poverbec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*tokenlstnew(char	*content)
{
	...
	if (!token)
		return (NULL);
	loop_until_space_or_operator(&content);
...
bool tokeniser(char *line)
{
	...
	free(line);
	iter_tokenlst(token_lst, print_tokenlst(token_lst));
	return (true);
}
